...
{
    /// <summary>
    /// The Node Editor block: up to <see cref="MaxRows"/> of Besiege's logic
    ...
    /// </summary>
    public class NodeEditorBehaviour : BlockModuleBehaviour<NodeEditorModule>
    {
        ...
        private string Numbered()
        {
            ...
            NodeEditorBehaviour[] all =
                FindObjectsOfType<NodeEditorBehaviour>();
            for (int n = 1; n < 100; n++)
            {
        ...
        /// <summary>Whether another Node Editor block on the machine is already
        /// using this prefix.</summary>
        private bool Elsewhere(NodeEditorBehaviour[] all, string wanted)
        {
        ...
        public MText Claim()
        {
            ...
            if (!string.IsNullOrEmpty(mine)
                && !Elsewhere(FindObjectsOfType<NodeEditorBehaviour>(), mine))
            {
